...
export class MerkleTree {
    ...
    async setNode(mtIndex, value) {
        ...
        if (this.currentSnapshotIdx === undefined) {
            ...
            await this.db.updatePathLogging(mtIndex + "I", oldDoc?.field !== undefined
                ? oldDoc.field
                : MerkleTree.emptyNodeHash(mtIndex.length), value, oldDoc?.snapshot ?? default_snapshot_id, this.currentSnapshotIdx);
        }
        this.cache.set(mtIndex, value);
    }
    async startSnapshot(id) {
        if (this.inMemoryMerkleTree !== undefined) {
            return;
        }
    ...
    async loadSnapshot(latest_snapshot) {
        if (this.inMemoryMerkleTree !== undefined) {
            return;
        }
    ...
    async getNodeOrDefault(mtIndex) {
    ...
    async getPath(index) {
        ...
        for (let i = 0; i < MaxHeight; i++) {
            ...
            Array.from(Array(BlockSize).keys()).map((v) => this.getNodeOrDefault(mtIndex.slice(0, i) + v)));
            ret.pathDigests.push(digests);
        }
        return ret;
    }
    async getLeave(index) {
        const mtIndex = this.convertToMtIndex(index);
        return await this.getNodeOrDefault(mtIndex);
    }
    async getChildren(mtIndex) {
        return await Promise.all(Array.from(Array(BlockSize).keys()).map((v) => this.getNodeOrDefault(mtIndex + v)));
    }
    async getLeaves(index) {
        const mtIndex = this.convertToMtIndex(index);
        return this.getChildren(mtIndex.slice(0, MaxHeight - 1));
    }
    async updateHash(index) {
        const mtIndex = this.convertToMtIndex(index);
        for (let i = 0; i < MaxHeight; i++) {
    ...
    async setLeaves(index, values) {
        ...
        await Promise.all(
        // Used to generate [0, 1, ..., BlockSize]
        Array.from(Array(BlockSize).keys()).map((v) => this.setNode(mtIndex.slice(0, MaxHeight - 1) + v, values[v])));
        await this.updateHash(index);
        return path;
    }
}
