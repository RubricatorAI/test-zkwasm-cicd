...
function applySparseMatrix(matrix, state) {
    const words = state.map((value) => new field_js_1.Field(value.v));
    let state0 = new field_js_1.Field(0);
    for (let i = 0; i < words.length; i++) {
        // console.log("applySparseMatrix", i, trimHex(matrix.row[i]));
        const f = new field_js_1.Field(new bn_js_1.default(trimHex(matrix.row[i]), 'hex'));
        state0 = state0.add(f.mul(words[i]));
    }
    state[0] = state0;
    for (let i = 1; i < words.length; i++) {
        const hat = new field_js_1.Field(new bn_js_1.default(trimHex(matrix.col_hat[i - 1]), 'hex'));
...
function toFieldArray(arr) {
    return arr.map((value) => new field_js_1.Field(new bn_js_1.default(trimHex(value), 'hex')));
}
function toFieldMatrix(arr) {
...
class Poseidon {
    constructor(config) {
        this.config = config;
        this.state = Array(this.config.t).fill(new field_js_1.Field(0));
        this.state[0] = new field_js_1.Field(new bn_js_1.default('0000000000000000000000000000000000000000000000010000000000000000', 16));
        this.absortbing = [];
        this.squeezed = false;
    }
    getState() {
    ...
    permute() {
        ...
        // Partial rounds
        {
            for (let i = 0; i < this.config.constants.partial.length && i < this.config.mds_matrices.sparse_matrices.length; i++) {
                ...
                this.state[0] = this.state[0].add(new field_js_1.Field(new bn_js_1.default(trimHex(this.config.constants.partial[i]), 'hex')));
    ...
    squeeze() {
        ...
        lastChunk.push(new field_js_1.Field(1));
...
function poseidon(inputs) {
    ...
    const hasher = new Poseidon(config_js_1.config);
