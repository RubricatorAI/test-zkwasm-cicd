...
export class TransactionData {
    constructor(nonce, command, params) {
        this.nonce = nonce;
...
export function createCommand(nonce, command, params) {
...
export function createWithdrawCommand(nonce, commandWithdraw, address, tokenIndex, amount) {
    ...
    */
    return createCommand(nonce, commandWithdraw, [tokenIndex, (firstLimb << 32n) + amount, sndLimb, thirdLimb]);
}
export class PlayerConvention {
    constructor(key, rpc, commandDeposit, commandWithdraw) {
    ...
    async deposit(pid_1, pid_2, tokenIndex, amount) {
        let nonce = await this.getNonce();
        try {
            const state = await this.rpc.sendTransaction(createCommand(nonce, this.commandDeposit, [pid_1, pid_2, tokenIndex, amount]), this.processingKey);
            return state;
        }
        catch (e) {
    ...
    async withdrawRewards(address, tokenIndex, amount) {
        let nonce = await this.getNonce();
        try {
            const state = await this.rpc.sendTransaction(createWithdrawCommand(nonce, this.commandWithdraw, address, tokenIndex, amount), this.processingKey);
            return state;
        }
        catch (e) {
            if (e instanceof Error) {
