...
export async function storeAccount(pkx, data, indexer) {
    const index = indexer(data);
    await accountModel.findOneAndUpdate({ pkx: pkx }, // Filter: finds the document by key
    { $set: {
            index: index,
            data: data
...
export async function queryAccounts(start) {
    try {
