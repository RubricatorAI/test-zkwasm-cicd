...
import { submitProofWithRetry, has_uncomplete_task, get_latest_proof, has_task } from "./prover.js";
import cors from "cors";
import { get_mongoose_db, modelBundle, modelJob, modelRand, get_service_port, get_server_admin_key, modelTx, get_contract_addr } from "./config.js";
...
export class Service {
    constructor(cb = async (arg, events) => { }, txBatched = async (arg, merkleHexRoot, postMerkleRoot) => { }, registerAPICallback = (app) => { }, registerPlayerIndexer = (player) => { return 0; }) {
        this.worker = null;
        this.queue = null;
        this.txCallback = cb;
    ...
    async findBundleIndex(merkleRoot) {
        try {
            const prevBundle = await modelBundle.findOne({
    ...
    async install_transactions(tx, jobid, events, isReplay = false) {
    ...
    async serve() {
        ...
        app.get('/data/players/:start?', async (req, res) => {
        ...
        app.get('/data/bundles/:merkleroot?', async (req, res) => {
